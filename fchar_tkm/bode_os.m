...
%Анализ систем с плотным спектром осцилляторов bode_os(Name,gr1,gr2,gr3) 
...
Nd=20;         % Мин. число точек на декаду частоты
rab=3^0.5/2;
arrow=2*[-rab+i/2;0;-rab-i/2;-rab+i/2]; %Фигура стрелки графика
%--------------------------------------------------------------------------%
Ts=Name.ts;    
...
%            disp(['N=',int2str(Gout(nfc-1,1))]);
        ...
        %disp(['Возврат     ' num2str(nfc) '    ' num2str(Gout(nfc,2)) ...
        %        '     ' int2str(Ndt)]);
        lt=lt-1/Ndt;
        Ndt=round(Ndt*dlrt/dlrc);	%% dlrc
        lt=lt+1/Ndt;
     end                % Алгоритм изменениЯ величины шага
  else                                      % nfc=1
     lt=lt+1/Ndt;
     nfc=nfc+1;
  end
...
%------------------------------------------------------------------------------------------%
   ...
   figure%(1)
     ...
     xlabel('Кругов. частота, 1/с','Fontname','Arial Unicode MS'); 
     ylabel([num2str(kga) '*A, дб,  F, град'],'Fontname','Arial Unicode MS'); grid;
      hold on
      if sum(size(XF))>0
         for j=1:length(XF(:,2))
   ...
   figure%(2)	
	...
	axis([0 rab1 -200 200]);	 axis(axis);     
   xlabel('Номер точки N','Fontname','Arial Unicode MS'); 
   ylabel([num2str(kga) '*A, дб,  F, град, 100*log10(om)'],'Fontname','Arial Unicode MS'); grid;
      hold on
      if sum(size(XF))>0
         for j=1:length(XF(:,2))
...
for j=2:nfc
   ...
   if (Gout(j,2)-1)*(Gout(j-1,2)-1)<=0	%Переход через om=1
	...
	if j>2
      ...
      if (rab1<0)&(Rgod(j)>10)	%Максимум радиуса
         plot(XsmY(j,1),XsmY(j,2),'.')
            ...
            Rotarr=(arrow*Ear).';
            patch(XsmY(j,1)+real(Rotarr),XsmY(j,2)+imag(Rotarr),Colar(scol+1,:));	% Стрелка	   
         if ~isempty(Mt)
