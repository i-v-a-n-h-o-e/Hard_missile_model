function boderus_A(Name,gr1,gr2,gr3)
...
Nd=20;%100;%500;%1000;%40;         % Мин. число точек на декаду частоты
rab=3^0.5/2;
...
PS=length(P);
if Tm>0
    Pz=pole(delay2z(feedback(Name,-1)));
    PZ=length(Pz);
    D1=sort(abs(P-1));
    q=1; d1=0;
    while d1<1e-5
...
while gm<=length(om_mx)
    if n<=2
        ...
        if Tm>0
            G1=evalfr(Name,exp(i*ome*Tm));%feval(ProbA,p);
        else
            G1=evalfr(Name,i*ome);
        end
        ...
        if ~isempty(Rez)&&(omea==Rez(1,1))
            ...
            if Tm>0
                G1=evalfr(Name,exp(i*omea*Tm));%feval(ProbA,p);
            else
                G1=evalfr(Name,i*omea);
            end
        end
        Kuv=1;
...
%     Vaxis=[-90 40 -50 50];
    ...
    for j=1:n
      ...
      patch(rab2-nidA,rab3,[0.76,0.76,0.76],'EdgeColor','none')	% Контур окрестности крит. точки
