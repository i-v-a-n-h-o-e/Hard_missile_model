%FCHAR5 - Программа расчёта частотных характеристик систем (аналоговых 
%и дискретных с одной частотой прерывания)
clc
disp('       Программа расчёта частотных характеристик дискретной системы ')
disp('                   с одной частотой прерывания FCHAR5')
clear all
clear global
global  INQ TIN
%i=(-1)^0.5;
dlr=6;         % Ном. расстояние между точками полярн. графика, дб
obA=20;        % Верхний уровень амплитуд полярн. графика, дб
...
ProbD=' ';
3^0.5/2;
arrow=2*[-ans+i/2;0;-ans-i/2;-ans+i/2];
%------------------------------------------------------------------------------------------%
load fcharust
disp(' ')
disp([blanks(10),'Предыдущие установки fchar: omn=',num2str(omn)])
disp(['                                      omk=',num2str(omk)])
disp(['                                       Nd=',num2str(Nd)])
rab1=input('Меняем? (y/<Enter>): ','s');
disp(' ')
if strcmp(rab1,'y')
	rab1=0;
	while rab1==0
        	omn=input('Введите начальную частоту omn=');
        	omk=input('Введите  конечную частоту omk=');
        	if omk>omn 
                	rab1=1;
        	else
           	disp(' ')
           	disp('Ошибка ввода. Введите omk>omn');
        	end
	end
	Nd=input('Введите минимальное число шагов на декаду Nd='); 
	disp(' ')
end
save fcharust omn omk Nd -append
disp([blanks(10),'Предыдущие установки fchar: '])
disp(['- имя m-файла  аналоговой структуры: ',ProbA])
disp(' ')
disp('Введите новое имя m-файла описания аналоговой структуры ');
ProbAin=input('(для сохранения прежнего - <Enter>): ','s');
disp(' ')
if length(ProbAin)>0 ProbA=ProbAin; end
tit=ProbA;
INQ=1;         % Запрос диалога из рабочего файла
C1=feval(ProbA,i*omn);
disp([blanks(10),'Предыдущие установки fchar: TIN=',num2str(TIN)])
 	disp(' ')
	disp('Введите новую величину периода прерывания TIN') 
...
if TIN>0
   disp(' ')
	...
	disp(' ')
   disp('Введите новое имя m-файла описания дискретной структуры ');
   ProbDin=input('(для сохранения прежнего - <Enter>): ','s');
   disp(' ')
   if length(ProbDin)>0 ProbD=ProbDin; end
	tit=[tit '  ' ProbD '  T=' num2str(TIN)];
        C1=feval(ProbD,C1,i*omn,TIN);      % Запрос диалога из рабочего файла
        if omk>pi/TIN
           omk=pi/TIN;
           disp('Значение omk ограничено половиной частоты прерывания')
           disp(' ')
        end
end
INQ=0;         % Запрос диалога из рабочего файла отключен
disp(' ')
disp([blanks(10),'Предыдущие установки fchar: DTEXT=',DTEXT])
disp(' ')
rab1=input('Меняем DTEXT? (y/<Enter>): ','s');
disp(' ')
if strcmp(rab1,'y')
   disp('Введите новый дополнительный текст в заголовок графика DTEXT')
...
while lt<=lk
  ...
  if TIN>0
       ...
       while abs(omsum)<8/TIN         % Ширина полосы частот
  ...
  if length(C1)>1
     error('Ошибка описания системы. Выход должен быть скаляром.');
  end
...
% Gout: [N,om,A,F(rad),Fsumangl(rad)]
  if nfc>1                          
%%%%%%%%%%%%%%%%%%%%%% Алгоритм изменения величины шага %%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     if Rad(1)>nidAst			%% dlrc - требуемая текущая dl
	     ...
	     dlrc=dlr/(1+2*(nidAst-Rad(1))/nidAst);	% В нуле dlrc уменьшен втрое
     end	
     dlrt=sqrt(Rad*Rad'-2*Rad(1)*Rad(2)*cos(Gout(nfc,4)-Gout(nfc-1,4)));
     if (dlrt<1.5*dlrc)|(Ndt>2000)  
         if (dlrt<1.5*dlrc) %Шаг годится
            Ndt=round(Ndt*dlrt/dlrc);	%% dlrc       
            if Ndt<Nd
               Ndt=Nd;
            end
         else  %Ndt>2000
            disp('Внимание! Особая точка на мнимой оси.')
            disp(['Omega=',num2str(Gout(nfc,2))])            
         end
        %-------------------% Алгоритм нахожд. точек A=0, F=0 %--------------------%
                 % Gout: [N,om,A,F(rad),Fsumangl(rad)]
         if Gout(nfc,3)>=-(nidA+10)
            ...
            if (Gout(nfc,4)*Gout(nfc-1,4)<=0)&(abs(Gout(nfc,4))<pi/2)...
                ...
                rab4=Gout(nfc-1,3)+(Gout(nfc,3)-Gout(nfc-1,3))*rab1;	% А
                if rab4>=-30
                  %home
                  XA=[XA; rab3 rab4 rab2];           % om A N
                  disp(['N=',int2str(Gout(nfc-1,1))])
               end
            end   %% Алгоритм нахожд. точек A=0, F=0 
            ...
            end  %% Алгоритм контроля вх. в зону            
        ...
        lt=lt+1/Ndt;
     end     %if dlrt<1.5*dlrc   % Алгоритм изменения величины шага
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
  else                                      % nfc=1
     lt=lt+1/Ndt;
     nfc=nfc+1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Dat=fix(clock);
disp('         Обработка данных для построения графиков.')
...
axis('normal');
      Nf(2)=gcf;
      rab1=log10(nfc)-floor(log10(nfc));    % Опр. масшт. по оси Х
...
for j=1:nfc
   if Gout(j,3)+nidA>0		% Gout(j,3) - 20*log10(A)
	...
	Rgod(j)=rab1;			% Радиус
   XsmY(j,1)=rab1*cos(Gout(j,4))-nidA;  %К-ты графика (декарт. к-ты 
   XsmY(j,2)=rab1*sin(Gout(j,4));       %относительно крит. точки)
   %------------------------------------------------------------------------------------------%
                     %Исключение перечеркиваний
   if  XsmY(j,1)<Vaxis(1)
      XsmY(j,1)=Vaxis(1);
...
for j=1:25
      ...
      patch((rab2-nidA)*2/3,rab3*2/3,'w','EdgeColor','none')	% Контур окрестности крит. точки
		%plot(rab2-nidA,rab3,'y.')   % Контур окрестности крит. точки
      rab1=[-80,-70,-50:10:-10,10,20,30]; rab2=zeros(size(rab1));		% Метки 10 дб +
      ...
      %plot(0, 0, '+r','Markersize',12)  %Метка в крит. точке
      plot(0, 0, '.r','Markersize',8)  %Метка в крит. точке
      
      %%%%%%%%%%%%%%%%%%%% Основной график %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      
      Mt1=-54: 12:-6; Mt2=43*ones(1,5);
      Mt=[Mt1,Mt1; Mt2,-Mt2];	% Массив координат выносок значений частот
      Col=['r  ';'k: ';'m* ';'c--';'b: ';'y  ';'g--'];
...
for j=2:nfc
   ...
   if ((rab1<0)&(Rgod(j-1)-Rgod(j-2)>0)&(Rgod(j)>10))|(j==nfc)	%Максимум радиуса
	     Vr=[Vr,XsmY(j,:)'];
	     scol=rem(s,Ncl)+1;
         %plot(Vr(1,:),Vr(2,:),'r'); 
     ...
     if Rgod(j)>10
         ...
         Rotarr=(arrow*Ear).';
         patch(XsmY(j,1)+real(Rotarr),XsmY(j,2)+imag(Rotarr),Colar(scol,:));	% Стрелка	   
 		   if ~isempty(Mt)
            [L,k]=min((XsmY(j,1)-Mt(1,:)).^2+(XsmY(j,2)-Mt(2,:)).^2);
            plot([XsmY(j,1),Mt(1,k)],[XsmY(j,2),Mt(2,k)],':m') %Линии выносок
            plot(Mt(1,k),Mt(2,k),'.')
            text(Mt(1,k),Mt(2,k)+3*sign(Mt(2,k)),num2str(round(Gout(j,2)*10)/10));
            rab=length(Mt(1,:));
            if rab==1
               Mt=[];
            else	%Вырезание использованной выноски
               switch k
   ...
   if (Gout(j,2)-1)*(Gout(j-1,2)-1)<=0	%Переход через om=1
      ((XsmY(j,1)-XsmY(j-1,1))^2+(XsmY(j,2)-XsmY(j-1,2))^2)^0.5;
       Ear=((XsmY(j,1)-XsmY(j-1,1))+i*(XsmY(j,2)-XsmY(j-1,2)))/ans;
            Rotarr=(arrow*Ear).';
       patch(XsmY(j,1)+real(Rotarr),XsmY(j,2)+imag(Rotarr),Colar(1,:));	% Стрелка om=1
   end
end  %for j=2:nfc

 %------------------------------------------------------------------------------------%     

      text(-88,-46,['УП = ',num2str(SumAng),' рад'])
      if sum(size(XA))>0
      ...
      set(gca,'XTick',[],'YTick',[])
disp(' ')
disp('Для сохранения графиков используйте функцию figsav (hgsave для версий MATLAB R11, R12).')      
disp(' ')
disp('Поздравляю с расчётом по программе FCHAR5 !')
